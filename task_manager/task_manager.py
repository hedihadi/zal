...
class TaskManager:
    ...
    def _get_process_icon(self, pid):
        ...
        try:
            ...
            icon = icon.resize((50, 50))  # Resize the image to (25, 25)
    ...
    def _get_running_processes_with_icon(self):
        processes = {}
        rawProcesses = psutil.process_iter(
        ...
        for process in rawProcesses:
            try:
                ...
                if any(substring in name for substring in ['System Idle Process', 'svchost', 'conhost']):
                    continue
                # Convert to MB
                memoryUsage = process_info['memory_info'].rss / (1024 * 1024)
                cpuPercent = process_info['cpu_percent']/cpu_count
                icon = self._get_process_icon(pid)
                if name not in processes:
                    processes[name] = {'pids': [
                        pid], 'memoryUsage': memoryUsage, 'cpuPercent': cpuPercent, 'icon': icon}
